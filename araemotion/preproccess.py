...
def simple_clean(text: str,
    ...
    if emoji_placehoders:
        text = text.replace("[emoji_start]","[EMOJI_START]").replace("[emoji_end]","[EMOJI_END]")
        
    for orig_value, replace_value in replacement_dict.items():
        text = text.replace(orig_value, replace_value)
